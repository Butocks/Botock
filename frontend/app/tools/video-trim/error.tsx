...
export default function VideoTrimError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("Video Trim Tool crashed:", error);
  }, [error]);
  ...
  const handleReset = async () => {
    try {
      ...
      // Ignore termination error on reset
    }
    reset();
  };
  ...
  return (
    <div className="max-w-2xl mx-auto py-20 px-4 text-center">
      ...
      <p className="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed">
        An error occurred while trimming your video. This may happen if the video format is unsupported,
        the video stream is corrupted, or the file size exceeded browser WebAssembly memory limits.
