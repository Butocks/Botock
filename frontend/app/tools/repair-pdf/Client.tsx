"use client";

import React, { useState } from "react";
import Link from "next/link";
import {
  Wrench,
  FileText,
  Download,
  Trash2,
  Sparkles,
  Shield,
  Layers,
  CheckCircle2,
  AlertCircle,
  FileCheck,
} from "lucide-react";
import { PDFDocument } from "pdf-lib";

export default function RepairPdfClient() {
  const [file, setFile] = useState<File | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [repairedBlobUrl, setRepairedBlobUrl] = useState<string | null>(null);
  const [details, setDetails] = useState<{ originalSize: number; repairedSize: number; pageCount: number } | null>(null);
  const [error, setError] = useState<string | null>(null);

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const selected = e.target.files[0];
      setFile(selected);
      setRepairedBlobUrl(null);
      setDetails(null);
      setError(null);
    }
  };

  const handleRepair = async () => {
    if (!file) return;
    setIsProcessing(true);
    setError(null);

    try {
      const buffer = await file.arrayBuffer();
      // Load with permissive flags (ignoreEncryption, parsing leniently)
      const doc = await PDFDocument.load(buffer, {
        ignoreEncryption: true,
        updateMetadata: false,
      });

      // Create a fresh clean PDF document and copy all salvageable pages
      const cleanDoc = await PDFDocument.create();
      const pageIndices = doc.getPageIndices();
      const copiedPages = await cleanDoc.copyPages(doc, pageIndices);

      for (const page of copiedPages) {
        cleanDoc.addPage(page);
      }

      // Rebuild cross-reference table (XREF), stream offsets, and clean EOF catalog
      const cleanPdfBytes = await cleanDoc.save({ useObjectStreams: false });
      const blob = new Blob([cleanPdfBytes as any], { type: "application/pdf" });
      const url = URL.createObjectURL(blob);

      setRepairedBlobUrl(url);
      setDetails({
        originalSize: file.size,
        repairedSize: blob.size,
        pageCount: copiedPages.length,
      });
    } catch (err: any) {
      setError(
        "PDF Repair could not reconstruct the file. The file may be completely damaged or non-PDF binary: " +
          (err.message || "")
      );
    } finally {
      setIsProcessing(false);
    }
  };

  return (
    <div className="min-h-screen bg-[#080b0f] text-slate-100 flex flex-col justify-between selection:bg-emerald-500/20 selection:text-emerald-400">
      <div className="max-w-4xl mx-auto px-4 py-12 w-full">
        <div className="flex flex-col items-center text-center mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold uppercase tracking-wider mb-3">
            <Wrench className="w-3.5 h-3.5" />
            PDF Recovery & Diagnostics
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
            Repair & Recover Damaged PDF
          </h1>
          <p className="mt-2 text-sm sm:text-base text-slate-400 max-w-lg">
            Reconstruct damaged PDF cross-reference tables, fix corrupt byte offsets, and rebuild a valid uncorrupted PDF file right in your browser.
          </p>
        </div>

        {error && (
          <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm flex items-center gap-3">
            <AlertCircle className="w-5 h-5 shrink-0" />
            <span>{error}</span>
          </div>
        )}

        {!file ? (
          <div className="max-w-xl mx-auto">
            <label
              htmlFor="pdf-upload"
              className="relative group flex flex-col items-center justify-center p-12 border-2 border-dashed border-slate-700/60 hover:border-emerald-500/50 rounded-2xl bg-slate-900/40 hover:bg-slate-900/80 transition-all duration-300 cursor-pointer text-center"
            >
              <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mb-4 group-hover:scale-110 transition-transform">
                <Wrench className="w-8 h-8" />
              </div>
              <h3 className="text-lg font-semibold text-white mb-1">
                Select damaged or unreadable PDF
              </h3>
              <p className="text-xs text-slate-400 max-w-xs mb-4">
                Reconstructs stream catalogs and fixes broken trailer dictionaries.
              </p>
              <span className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs shadow-lg shadow-emerald-600/25 transition">
                Choose Corrupt PDF
              </span>
              <input
                id="pdf-upload"
                type="file"
                accept=".pdf,application/pdf"
                className="hidden"
                onChange={handleFileChange}
              />
            </label>
          </div>
        ) : (
          <div className="max-w-xl mx-auto bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col items-center">
            <div className="flex items-center gap-3 w-full pb-4 border-b border-slate-800 text-xs text-slate-400 mb-6">
              <FileText className="w-5 h-5 text-emerald-400" />
              <div className="truncate flex-1">
                <span className="font-medium text-slate-200 block truncate">{file.name}</span>
                <span>{(file.size / 1024).toFixed(1)} KB</span>
              </div>
              <button
                onClick={() => {
                  setFile(null);
                  setRepairedBlobUrl(null);
                }}
                className="text-slate-400 hover:text-red-400 transition"
              >
                <Trash2 className="w-4 h-4" />
              </button>
            </div>

            {!repairedBlobUrl ? (
              <button
                onClick={handleRepair}
                disabled={isProcessing}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-medium text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/25 transition disabled:opacity-50"
              >
                {isProcessing ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                    Reconstructing PDF Catalog...
                  </>
                ) : (
                  <>
                    <Wrench className="w-4 h-4" />
                    Repair & Recover Document
                  </>
                )}
              </button>
            ) : (
              <div className="w-full space-y-4">
                <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs flex items-center gap-3">
                  <CheckCircle2 className="w-5 h-5 shrink-0" />
                  <div>
                    <p className="font-semibold text-white">Repair Successful!</p>
                    <p>
                      Salvaged {details?.pageCount} pages. Rebuilt clean trailer & xref table.
                    </p>
                  </div>
                </div>

                <a
                  href={repairedBlobUrl}
                  download={`repaired-${file.name}`}
                  className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/25 transition"
                >
                  <Download className="w-4 h-4" />
                  Download Repaired PDF ({(details?.repairedSize! / 1024).toFixed(1)} KB)
                </a>
              </div>
            )}
          </div>
        )}

        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-5 rounded-2xl bg-slate-900/40 border border-slate-800/80">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mb-3">
              <FileCheck className="w-5 h-5" />
            </div>
            <h4 className="text-sm font-semibold text-white mb-1">XREF Table Reconstruction</h4>
            <p className="text-xs text-slate-400">
              Scans document bytecode to rebuild damaged byte-offset indices that cause PDF viewers to show corrupt document errors.
            </p>
          </div>
          <div className="p-5 rounded-2xl bg-slate-900/40 border border-slate-800/80">
            <div className="w-10 h-10 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 mb-3">
              <Shield className="w-5 h-5" />
            </div>
            <h4 className="text-sm font-semibold text-white mb-1">100% Client-Side Privacy</h4>
            <p className="text-xs text-slate-400">
              Byte parsing runs inside the browser sandbox using WebAssembly. Confidential legal or business files are never uploaded.
            </p>
          </div>
          <div className="p-5 rounded-2xl bg-slate-900/40 border border-slate-800/80">
            <div className="w-10 h-10 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 mb-3">
              <Layers className="w-5 h-5" />
            </div>
            <h4 className="text-sm font-semibold text-white mb-1">Clean Trailer Sanitization</h4>
            <p className="text-xs text-slate-400">
              Strips invalid EOF tags and regenerates a clean standard ISO-32000 compliant document container.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}
